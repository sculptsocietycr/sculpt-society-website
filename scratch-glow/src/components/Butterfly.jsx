...
/**
 * Mariposa SVG brand-colored — idéntica a la de sculptsocietycr.com.
 *
 * Props:
 *   color      → cualquier color brand (orange, wine, pink, blush, gold)
 *   size       → px (default 40)
 *   className  → para posicionar con Tailwind
 *   flapMs     → velocidad de aleteo (default 600)
 *   driftX/Y   → desplazamiento al scrollear (px acumulados)
 *   rotate     → rotación inicial
 *   spin       → grados de rotación adicional con el scroll
 *   flip       → espejar horizontalmente
 *   delay      → desfase del aleteo (s)
 */
export default function Butterfly({
  ...
  const rot = useTransform(
    scrollY,
    range,
    reduce ? [rotate, rotate] : [rotate, rotate + spin]
  );
